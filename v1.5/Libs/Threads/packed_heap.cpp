...
void PackedAllocator::Free( idx_t idx )
{
  ...
  {
	[[maybe_unused]] size_t ret = freeRegionsTree [ aboveRegion->size ].erase( end );
  ...
  {
	[[maybe_unused]] size_t ret = freeRegionsTree [ belowRegion->size ].erase( start - 2 * sizeof( MetaData ) - belowRegion->size );
