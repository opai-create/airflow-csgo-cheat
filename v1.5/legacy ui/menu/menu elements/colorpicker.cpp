...
void RenderColorRectWithAlphaCheckerboard_Wrapper(ImVec2 p_min, ImVec2 p_max, c_color clr, float grid_step, ImVec2 grid_off, float rounding, int rounding_corners_flags, float alpha_mod = 1.f)
{
	ImGuiWindow* window = GetCurrentWindow();
	c_color temp_col_bg1 = c_color(204, 204, 204).multiply(clr, clr.a() / 255.f).new_alpha(255 * alpha_mod);
	c_color temp_col_bg2 = c_color(128, 128, 128).multiply(clr, clr.a() / 255.f).new_alpha(255 * alpha_mod);
...
bool c_menu::color_picker_wrapper(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	auto& picker_mod = item_animations[CONST_HASH(label)];
	...
	BeginGroup();
	{
		...
		BeginGroup();
		{
			auto& copy_mod = item_animations[CONST_HASH(label) + HASH("Copy")];
			bool copy_hover = false;
			auto copy_button = ImGui::ButtonEx(CXOR("##copybutton"), ImVec2(76, 24), 0, &copy_hover);
			...
			c_color button_clr = c_color(35, 35, 35).increase(20 * copy_mod.hovered_alpha).new_alpha(button_clr_alpha);
			...
			draw_list->AddText(ImVec2(pos.x + button_size.x / 2 - 6.f, (pos.y + size.y) - button_size.y - 3.f), c_color(255, 255, 255, 255 * picker_alpha).as_imcolor(), CXOR("Copy"));

			if (copy_button)
			{
				std::string new_str = std::to_string(c_color(col[0] * 255.f, col[1] * 255.f, col[2] * 255.f, col[3] * 255.f).u32());
		...
		BeginGroup();
		{
			auto& paste_mod = item_animations[CONST_HASH(label) + HASH("Paste")];
			bool paste_hover = false;
			auto paste_button = ImGui::ButtonEx(CXOR("##pastebutton"), ImVec2(76, 24), 0, &paste_hover);
			...
			c_color button_clr = c_color(35, 35, 35).increase(20 * paste_mod.hovered_alpha).new_alpha(button_clr_alpha);
			...
			draw_list->AddText(ImVec2(base_pos + button_size.x / 2 - 6.f, (pos.y + size.y) - button_size.y - 3.f), c_color(255, 255, 255, 255 * picker_alpha).as_imcolor(), CXOR("Paste"));

			if (paste_button)
			{
				auto clip_clr = c_color(std::atoll(from_clipboard().c_str()));
				auto temp_clr = c_color::hsb(clip_clr.hue(), clip_clr.saturation(), clip_clr.brightness()).new_alpha(clip_clr.a());
...
bool c_menu::color_button_wrapper(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags)
{
	...
	auto& mod = item_animations[CONST_HASH(desc_id) + CONST_HASH("color")];
	...
	if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
	{
		...
		RenderColorRectWithAlphaCheckerboard_Wrapper(ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y), bb_inner.Max, c_color(col_rgb.x * 255, col_rgb.y * 255, col_rgb.z * 255, col_rgb.w * 255), grid_step,
	...
	else
	{
		...
		if (col_source.w < 1.0f)
			RenderColorRectWithAlphaCheckerboard_Wrapper(
				bb_inner.Min, bb_inner.Max, c_color(col_source.x * 255, col_source.y * 255, col_source.z * 255, col_source.w * 255), grid_step, ImVec2(off, off), rounding, ImDrawCornerFlags_All, alpha);
		else
...
bool c_menu::color_picker(const char* name, c_float_color& v, ImGuiColorEditFlags flags)
{
	...
	if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
	{
		...
		std::string button_str = std::string(name) + XOR("##color");
		...
		if (!(flags & ImGuiColorEditFlags_NoOptions) && ImGui::IsItemHovered() && IsMouseClicked(1))
			OpenPopup(CXOR("context"));

		bool begin_popup = BeginPopupEx(g.CurrentWindow->GetID(CXOR("picker")), ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove);

		std::string picker_name = std::string(name) + XOR("picker");
		auto& picker_mod = item_animations[CONST_HASH(picker_name.c_str())];
		this->create_animation(picker_mod.alpha, begin_popup, 0.3f, skip_disable | lerp_animation);

		auto base_pos = ImGui::GetWindowPos();
		ImGui::GetWindowDrawList()->AddRectFilled(base_pos, base_pos + ImGui::GetWindowSize(), c_color(16, 16, 16, 255 * window_alpha * picker_mod.alpha).as_imcolor(), 10.f);
