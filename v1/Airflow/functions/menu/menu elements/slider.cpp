...
static const char* patch_formant_string_float_to_int(const char* fmt)
{
	...
	if (fmt_end > fmt_start && fmt_end[-1] == 'f')
	{
		...
		ImFormatString(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), xor_c("%.*s%%d%s"), (int)(fmt_start - fmt), fmt, fmt_end); // Honor leading and trailing decorations, but lose alignment/precision.
		return g.TempBuffer;
	}
	return fmt;
}
...
bool c_menu::slider_scalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
	...
	else if (data_type == ImGuiDataType_S32 && strcmp(format, xor_c("%d")) != 0) // (FIXME-LEGACY: Patch old "%.0f" format string to use "%d", read function more details.)
	...
	color back_clr = color(217, 217, 217).increase(38 * mod_frame.hovered_alpha).new_alpha(back_alpha);
	...
	// slider body
	draw_list->AddRectFilled(render_bounds.Min, render_bounds.Max, color(0, 0, 0, 80 * alpha).as_imcolor(), 3.f);
	...
	if (grab_bb.Max.x > grab_bb.Min.x)
	{
		...
		draw_list->AddCircleFilled(circle_pos, circle_radius, color(75, 75, 75, 255 * alpha).as_imcolor());
		draw_list->AddCircleFilled(circle_pos, circle_radius - 3.f, color(255, 255, 255, 255 * alpha).as_imcolor());
	}
