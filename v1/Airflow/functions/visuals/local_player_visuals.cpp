...
{
	auto death_notice = (uintptr_t)func_ptrs::find_hud_element(*patterns::get_hud_ptr.as< uintptr_t** >(), xor_str("CCSGO_HudDeathNotice").c_str());
	if (!death_notice)
		return;

	static auto clear_notices = patterns::clear_killfeed.as< void(__thiscall*)(uintptr_t) >();
	static bool reset_killfeed = false;
	static auto next_update = 0.f;
	...
	if (next_update > interfaces::global_vars->real_time)
		return;

	next_update = interfaces::global_vars->real_time + 2.f;

	const auto panel = (*(c_ui_panel**)(*(uintptr_t*)(death_notice - 20 + 88) + sizeof(uintptr_t)));
	...
	for (int i = 0; i < count; ++i)
	{
		...
		if (child->has_class(xor_str("DeathNotice_Killer").c_str()))
			child->set_attribute_float(xor_str("SpawnTime").c_str(), interfaces::global_vars->cur_time);
	}
}
...
pen_data_t get_penetration_for_crosshair()
{
	vector3d dir{};
	math::angle_to_vectors(g_ctx.orig_angle, dir);

	auto start = g_ctx.local->get_eye_position();
	return g_auto_wall->fire_bullet(g_ctx.local, nullptr, g_ctx.weapon_info, g_ctx.weapon->is_taser(), start, start + (dir * g_ctx.weapon_info->range), true);
}
...
void c_local_visuals::on_paint_traverse()
{
	...
	if (g_ctx.in_game)
	{
		...
		g_ctx.max_unlag = cvars::sv_maxunlag->get_float();
		...
		if (g_ctx.weapon)
		{
			...
			if (!g_cfg.misc.menu)
			{
				int legit_tab = rage_tools::get_legit_tab();
			...
			{
				auto threaded_bullet = g_thread_pool->add_task(get_penetration_for_crosshair);
				auto result = std::any_cast<pen_data_t>(g_thread_pool->wait_result(threaded_bullet));
		
				g_ctx.can_penetrate = result.dmg > 0;
			}
			else
				g_ctx.can_penetrate = false;
		}
	...
	if (!peek_positions.empty())
	{
		...
		for (int i = 0; i < peek_positions.size() - 1; ++i) {
			g_render->filled_triangle_gradient(
				...
				peek_w2s.x, peek_w2s.y,
				clr.new_alpha(new_alpha), clr.new_alpha(new_alpha), clr.new_alpha(150.f - new_alpha));
		}
...
void c_local_visuals::on_render_start(int stage)
{
	...
	if (interfaces::input->camera_in_third_person)
		patterns::update_visibility.as< void(__thiscall*)(void*) >()(g_ctx.local);
