...
namespace math
{
	...
	{
		out = { in1.dot(vector3d(in2.mat[0][0], in2.mat[0][1], in2.mat[0][2])) + in2.mat[0][3], 
	...
	{
		return { in1.dot(vector3d(in2.mat[0][0], in2.mat[0][1], in2.mat[0][2])) + in2.mat[0][3], in1.dot(vector3d(in2.mat[1][0], in2.mat[1][1], in2.mat[1][2])) + in2.mat[1][3],
		  in1.dot(vector3d(in2.mat[2][0], in2.mat[2][1], in2.mat[2][2])) + in2.mat[2][3] };
	}
	...
	float segment_to_segment(const vector3d& s1, const vector3d& s2, vector3d& k1, vector3d& k2)
	{
		...
		return (w + u * sc - v * tc).length(false);
	}
	...
	float bias(float x, float bias_amt)
	{
		...
		{
			last_exponent = log(bias_amt) * -1.4427f;
		}
		return pow(x, last_exponent);
	}
