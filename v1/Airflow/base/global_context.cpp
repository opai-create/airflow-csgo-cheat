...
namespace interfaces
{
	...
	c_client_leaf_system* client_leaf_system{};
	void* file_system{};
	...
	__forceinline void init()
	{
		...
		weapon_system = *patterns::weapon_system.as< c_weapon_system** >();
		...
		glow_object_manager = *patterns::glow_object.as< c_glow_object_manager** >();
		client_state = **patterns::client_state.add(1).as< c_clientstate*** >();
		move_helper = **patterns::move_helper.as< c_movehelper*** >();
		file_system = **patterns::file_system_ptr.as< void*** >();
...
namespace cvars
{
	...
	c_cvar* cl_pred_doresetlatch{};
	c_cvar* cl_cmdrate{};
	c_cvar* rate{};
	c_cvar* r_DrawSpecificStaticProp{};
	c_cvar* sv_accelerate_use_weapon_speed{};
	c_cvar* sv_maxvelocity{};
	...
	__forceinline void init()
	{
		...
		sv_unlag = interfaces::convar->find_convar(xor_c_s("sv_unlag"));
		...
		cl_csm_max_shadow_dist = interfaces::convar->find_convar(xor_c_s("cl_csm_max_shadow_dist"));
		...
		cl_interpolate = interfaces::convar->find_convar(xor_c_s("cl_interpolate"));
		cl_pred_doresetlatch = interfaces::convar->find_convar(xor_c_s("cl_pred_doresetlatch"));
		cl_cmdrate = interfaces::convar->find_convar(xor_c_s("cl_cmdrate"));
		rate = interfaces::convar->find_convar(xor_c_s("rate"));
		r_DrawSpecificStaticProp = interfaces::convar->find_convar(xor_c_s("r_DrawSpecificStaticProp"));
		sv_accelerate_use_weapon_speed = interfaces::convar->find_convar(xor_c_s("sv_accelerate_use_weapon_speed"));
		sv_maxvelocity = interfaces::convar->find_convar(xor_c_s("sv_maxvelocity"));
	}
}
