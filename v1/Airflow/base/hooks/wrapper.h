...
namespace hooks
{
	namespace detour
	{
		...
		public:
			...
			__forceinline ret original(fn custom_func)
			{
	...
	namespace vmt
	{
		...
		{
		public:
			__forceinline c_protect_guard(void* base, uint32_t len, uint32_t protect)
			{
				this->base = base;
				this->len = len;
		...
		{
		public:
			__forceinline c_hooks() : class_base(nullptr), method_count(0), shadow_vtable(nullptr), original_vtable(nullptr), indexes({}), unhooked(false)
			{
			}

			__forceinline c_hooks(void* base) : class_base(base), method_count(0), shadow_vtable(nullptr), original_vtable(nullptr), indexes({}), unhooked(false)
			{
			}
			...
			template < typename T >
			__forceinline T original(uint32_t index)
			{
				return (T)original_vtable[index];
			}

			__forceinline void restore_vtable()
			{
				if (original_vtable != nullptr && (*original_vtable) != 0)
				{
					c_protect_guard guard = c_protect_guard{ class_base, sizeof(uintptr_t), PAGE_READWRITE };
			...
			__forceinline bool was_unhooked()
			{
				return unhooked;
			}
		...
		private:
			__forceinline uint32_t get_vtable_methods(uintptr_t* vtable_start)
			{
				uint32_t len = -1;

				while (c_address::valid(vtable_start[len]))
					len++;
