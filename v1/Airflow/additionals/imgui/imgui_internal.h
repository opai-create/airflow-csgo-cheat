...
struct ImGuiColorMod;               // Stacked color alpha, backup of modified data so we can restore it
...
struct ImGuiStyleMod;               // Stacked style alpha, backup of modified data so we can restore it
...
struct IMGUI_API ImPool
{
    ...
    void        remove_events(ImGuiID key, const T* p)     { remove_events(key, GetIndex(p)); }
    void        remove_events(ImGuiID key, ImPoolIdx idx)  { Buf[idx].~T(); *(int*)&Buf[idx] = FreeIdx; FreeIdx = idx; Map.SetInt(key, -1); }
...
enum ImGuiButtonFlags_
{
    ...
    ImGuiButtonFlags_NoKeyModifiers         = 1 << 10,  // disable mouse interaction if a key alpha is held
...
enum ImGuiSliderFlags_
{
    ...
    ImGuiSliderFlags_IgnoreGrabCalc         = 1 << 1
};
...
// Stacked color alpha, backup of modified data so we can restore it
...
// Stacked style alpha, backup of modified data so we can restore it. Data type inferred from the variable.
...
struct ImGuiContext
{
    ...
    ImGuiWindow*            OldWindow;                     // Track the window we started mouse-wheeling on. Until a timer elapse or mouse has moved, generally keep scrolling the same window even if during the course of scrolling the mouse ends up hovering a child window.
    ImGuiWindow*            OldWindowChild;                     // Track the window we started mouse-wheeling on. Until a timer elapse or mouse has moved, generally keep scrolling the same window even if during the course of scrolling the mouse ends up hovering a child window.
...
namespace ImGui
{
    ...
    // - You are calling ImGui functions after ImGui::EndFrame()/ImGui::on_directx() and before the next ImGui::NewFrame(), which is also illegal.
    ...
    IMGUI_API void          RenderColorRectWithAlphaCheckerboard(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, float grid_step, ImVec2 grid_off, float rounding = 0.0f, int rounding_corners_flags = ~0, float alpha_mod = 1.f);
    IMGUI_API void          RenderCheckMark(ImVec2 pos, ImU32 col, float sz);
    IMGUI_API void          RenderNavHighlight(const ImRect& bb, ImGuiID id, ImGuiNavHighlightFlags flags = ImGuiNavHighlightFlags_TypeDefault); // Navigation highlight
    IMGUI_API const char*   FindRenderedTextEnd(const char* text, const char* text_end = NULL); // Find the optional ## from which we stop displaying text.
    IMGUI_API void          LogRenderedText(const ImVec2* ref_pos, const char* text, const char* text_end = NULL);

    // on_directx helpers (those functions don't access any ImGui state!)
    ...
    IMGUI_API bool          ButtonEx(const char* label, const ImVec2& size_arg = ImVec2(0,0), ImGuiButtonFlags flags = 0, bool* hovered_ptr = nullptr);
	...
	IMGUI_API bool			SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags, ImVec4 color, ImVec2 valuesize, const char* label, char* value);
	...
	IMGUI_API bool			SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power, ImVec2 size, ImVec4 color);
