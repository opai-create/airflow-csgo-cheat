...
/*
 ...
 - C++: ImVec2/ImVec4 do not expose math operators by default, because it is expected that you use your own math types.
   See FAQ "How can interfaces use my own math types instead of ImVec2/ImVec4?" for details about setting up imconfig.h for that.
 ...
 - Dear ImGui never touches or knows about your GPU state. The only function that knows about GPU is the draw function that you provide.
   ...
   phases of your own application. All rendering information are stored into command-lists that you will retrieve after calling ImGui::on_directx().
 ...
 EXHIBIT 1: USING THE EXAMPLE BINDINGS (imgui_impl_XXX.cpp files from the examples/ folder).
     ...
     while (true)
     {
         ...
         // on_directx dear imgui into screen
         ImGui::on_directx();
 ...
 EXHIBIT 2: IMPLEMENTING CUSTOM BINDING / CUSTOM ENGINE
     ...
     while (true)
     {
        ...
        ImGui::Text("Hello, world!");
        MyGameUpdate(); // may use any Dear ImGui functions, e.g. ImGui::Begin("My window"); ImGui::Text("Hello, world!"); ImGui::finish();
        MyGameRender(); // may use any Dear ImGui functions as well!

        // on_directx dear imgui, swap buffers
        // (You want to try calling EndFrame/on_directx as late as you can, to be able to use Dear ImGui in your own game rendering code)
        ImGui::EndFrame();
        ImGui::on_directx();
       ...
       for (int n = 0; n < draw_data->CmdListsCount; n++)
       {
          ...
          for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
             ...
             else
             {
                 ...
                 // on_directx 'pcmd->ElemCount/3' indexed triangles.
 ...
 - 2018/02/16 (1.60) - obsoleted the io.RenderDrawListsFn callback, you can call your graphics engine render function after ImGui::on_directx(). Use ImGui::GetDrawData() to retrieve the ImDrawData* to display.
 ...
 - 2016/04/26 (1.49) - changed ImDrawList::PushClipRect(ImVec4 rect) to ImDrawList::PushClipRect(ImVector2D min,ImVec2 max,bool intersect_with_current_clip_rect=false). Note that higher-level ImGui::PushClipRect() is preferable because it will clip at logic/widget level, whereas ImDrawList::PushClipRect() only affect your renderer.
 ...
 Q: Which version should interfaces get?
 ...
 Q: How can interfaces tell whether to dispatch mouse/keyboard to Dear ImGui or to my application?
 ...
 Q. How can interfaces enable keyboard controls?
 Q: How can interfaces use this without a mouse, without a keyboard or without a screen? (gamepad, input share, remote display)
 Q: interfaces integrated Dear ImGui in my engine and the text or lines are blurry..
 Q: interfaces integrated Dear ImGui in my engine and some elements are clipping or disappearing when interfaces move windows around..
 >> See https://www.dearimgui.org/faq

 Q&A: Usage
 ----------

 Q: Why are multiple widgets reacting when interfaces interact with a single one?
 Q: How can interfaces have multiple widgets with the same label or with an empty label?
 ...
 Q: How can interfaces display an image? What is ImTextureID, how does it works?
 >> See https://www.dearimgui.org/faq and https://github.com/ocornut/imgui/wiki/Image-Loading-and-Displaying-Examples

 Q: How can interfaces use my own math types instead of ImVec2/ImVec4?
 Q: How can interfaces interact with standard C++ types (such as std::string and std::vector)?
 Q: How can interfaces display custom shapes? (using low-level ImDrawList API)
 >> See https://www.dearimgui.org/faq

 Q&A: Fonts, Text
 ================

 Q: How can interfaces load a different font than the default?
 Q: How can interfaces easily use icons in my application?
 Q: How can interfaces load multiple fonts?
 Q: How can interfaces display and input non-Latin characters such as Chinese, Japanese, Korean, Cyrillic?
 >> See https://www.dearimgui.org/faq and docs/FONTS.txt

 Q&A: Community
 ==============

 Q: How can interfaces help?
...
// FIXME-LEGACY: Ideally we should remove the Begin/finish functions but they are part of the legacy API we still support. This is why some of the code in Step() calling Begin() and reassign some fields, spaghetti style.
...
void ImGui::RenderTextEllipsis(ImDrawList* draw_list, const ImVec2& pos_min, const ImVec2& pos_max, float clip_max_x, float ellipsis_max_x, const char* text, const char* text_end_full, const ImVec2* text_size_if_known)
{
    ...
    if (text_size.x > pos_max.x - pos_min.x)
    {
        ...
        // on_directx text, render ellipsis
...
// on_directx a rectangle shaped with optional rounding and borders
...
// on_directx an arrow aimed to be aligned with text (p_min is a position in the same space text would be positioned). To e.g. denote expanded/collapsed state
    ...
    : DrawListInst(&context->DrawListSharedData)
{
    ...
    ScrollLerp = ImVec2(0.f, 0.f);
    ScrollNext = ImVec2(0.f, 0.f);
...
bool ImGui::IsItemHovered(ImGuiHoveredFlags flags)
{
    ...
    // [2017/10/16] Reverted commit 344d48be3 and testing RootWindow instead. interfaces believe it is correct to NOT test for RootWindow but this leaves us unable to use IsItemHovered() after EndChild() itself.
    // Until a solution is found interfaces believe reverting to the test from 2017/09/27 is safe since this was the test that has been running for a long while.
...
bool ImGui::DebugCheckVersionAndDataLayout(const char* version, size_t sz_io, size_t sz_style, size_t sz_Vector2D, size_t sz_vec4, size_t sz_vert, size_t sz_idx)
{
    bool error = false;
    if (strcmp(version, IMGUI_VERSION)!=0) { error = true; IM_ASSERT(strcmp(version,IMGUI_VERSION)==0 && "Mismatched version string!");  }
    if (sz_io    != sizeof(ImGuiIO))       { error = true; IM_ASSERT(sz_io    == sizeof(ImGuiIO)      && "Mismatched struct layout!"); }
    if (sz_style != sizeof(ImGuiStyle))    { error = true; IM_ASSERT(sz_style == sizeof(ImGuiStyle)   && "Mismatched struct layout!"); }
    if (sz_Vector2D  != sizeof(ImVec2))        { error = true; IM_ASSERT(sz_Vector2D  == sizeof(ImVec2)       && "Mismatched struct layout!"); }
...
// Same value as passed to the old io.RenderDrawListsFn function. Valid after on_directx() and until the next call to NewFrame()
...
void ImGui::NewFrame()
{
    ...
    // But in order to allow the user to call NewFrame() multiple times without calling on_directx(), we are doing an explicit clear.
...
// This is normally called by on_directx(). You may want to call it directly if you want to avoid calling on_directx() but the gain will be very minimal.
void ImGui::EndFrame()
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(g.Initialized);
...
void ImGui::Render()
{
    ...
    // (Legacy) Call the on_directx callback function. The current prefer way is to let the user retrieve GetDrawData() and call the render function themselves.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
void ImGui::EndChild(bool fade_out)
{
    ImGuiWindow* window = GetCurrentWindow();
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
        ...
        // on_directx resize grips (after their input handling so we don't have a frame of latency)
...
// on_directx title text, collapse button, close button
...
// - A default window called "Debug" is automatically stacked at the beginning of every frame so you can use widgets without explicitly calling a Begin/finish pair.
// - Begin/finish can be called multiple times during the frame with the same window name to append content.
// - The window name is used as a unique identifier to preserve window information across frames (and save rudimentary information to the .ini file).
//   You can use the "##" or "###" markers to use the same label with different id, or same id with different label. See documentation at the top of this file.
// - Return false when window is collapsed, so you can early out in your code. You always need to call ImGui::finish() even if false is returned.
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    IM_ASSERT(g.WithinFrameScope);                  // Forgot to call ImGui::NewFrame()
    IM_ASSERT(g.FrameCountEnded != g.FrameCount);   // Called ImGui::on_directx() or ImGui::EndFrame() and haven't called ImGui::NewFrame() again yet
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // Apply scrolling
        float needed_scroll = CalcNextScrollFromScrollTargetAndClamp(window, false).y;
        ...
        // фикс скролл бара ебать))
        else {

            window->Scroll = CalcNextScrollFromScrollTargetAndClamp(window, false);
        ...
        // This works but 1. doesn't handle multiple Begin/finish pairs, 2. recursing into another Begin/finish pair - so we need to work that out and add better logging scope.
...
bool ImGui::IsWindowFocused(ImGuiFocusedFlags flags)
{
    ...
    IM_ASSERT(g.CurrentWindow);     // Not inside a Begin()/finish()
...
{
    // Report when there is a mismatch of Begin/BeginChild vs finish/EndChild calls. Important: Remember that the Begin/BeginChild API requires you
    // to always call finish/EndChild even if Begin/BeginChild returns false! (this is unfortunately inconsistent with most other Begin* API).
    ImGuiContext& g = *GImGui;
...
// Save and compare stack sizes on Begin()/finish() to detect usage errors
// Begin() calls this with write=true
// finish() calls this with write=false
static void ImGui::ErrorCheckBeginEndCompareStacksSize(ImGuiWindow* window, bool write)
{
    ...
    // Global stacks
    // For color, style and font stacks there is an incentive to use Push/Begin/Pop/.../finish patterns, so we relax our checks a little to allow them.
...
{
    ImGuiContext& g = *GImGui;
    //if (!g.IO.NavActive)  // [2017/10/06] Removed this possibly redundant test but interfaces am not sure of all the side-effects yet. Some of the feature here will need to work regardless of using a _NoNavInputs flag.
    //    return;
...
static void ImGui::NavUpdate()
{
    ...
    // Update PageUp/PageDown/Home/finish scroll
...
// Handle PageUp/PageDown/Home/finish keys
static float ImGui::NavUpdatePageUpPageDown()
{
    ...
    if (page_up_held != page_down_held || home_pressed != end_pressed) // If either (not both) are pressed
    {
        ...
        else
        {
            ...
            {
                // FIXME-NAV: handling of Home/finish is assuming that the top/bottom most item will be visible with Scroll.y == 0/ScrollMax.y
...
const ImGuiPayload* ImGui::AcceptDragDropPayload(const char* type, ImGuiDragDropFlags flags)
{
    ...
    // on_directx default drop visuals
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
        ...
        static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list, const char* label)
        {
            ...
            ImDrawList* fg_draw_list = GetForegroundDrawList(window); // on_directx additional visuals into the top-most draw list
...
ImDrawPaint& ImDrawPaint::SetLinearGradient(const ImVec2& start, const ImVec2& end, const ImVec4& col0, const ImVec4& col1, const float& angle)
{
    Col0 = col0;
    Col1 = col1;
    Start = start;
    auto size = end - start;
    auto end_ = angle >= 0.f ? ImRotate(end, static_cast<float>(cos(angle)), static_cast<float>(sin(angle))) : end;
    Extent.x = end_.x - start.x;
    Extent.y = end_.y - start.y;
    return *this;
}


ImU32 ImDrawPaint::Calc(const ImVec2& pos) const
{
    auto delta = pos - Start;
    auto deltaLen = delta.x / Extent.x;
    //auto mainOffset = ImVec2(1.f, 1.f).Length2D();
    auto persenge = deltaLen;//deltaLen >= 0.f ? deltaLen / mainOffset : 0.f;
    persenge = persenge > 1.f ? 1.f : persenge;
    return ImColor(
        Col0.x + (Col1.x - Col0.x) * persenge,
        Col0.y + (Col1.y - Col0.y) * persenge,
        Col0.z + (Col1.z - Col0.z) * persenge,
        Col0.w + (Col1.w - Col0.w) * persenge);
}
