...
struct ImGuiIO;                     // Main configuration and interfaces/O between your application and ImGui
...
namespace ImGui
{
    ...
    IMGUI_API bool          DebugCheckVersionAndDataLayout(const char* version_str, size_t sz_io, size_t sz_style, size_t sz_Vector2D, size_t sz_vec4, size_t sz_drawvert, size_t sz_drawidx);

    // Main
    IMGUI_API ImGuiIO&      GetIO();                                    // access the IO structure (mouse/keyboard/gamepad inputs, time, various configuration options/flags)
    IMGUI_API ImGuiStyle&   GetStyle();                                 // access the Style structure (colors, sizes). Always use PushStyleCol(), PushStyleVar() to modify style mid-frame.
    IMGUI_API void          NewFrame();                                 // start a new Dear ImGui frame, you can submit any command from this point until on_directx()/EndFrame().
    IMGUI_API void          EndFrame();                                 // ends the Dear ImGui frame. automatically called by on_directx(), you likely don't need to call that yourself directly. If you don't need to render data (skipping rendering) you may call EndFrame() but you'll have wasted CPU already! If you don't need to render, better to not create any imgui windows and not call NewFrame() at all!
    ...
    IMGUI_API ImDrawData*   GetDrawData();                              // valid after on_directx() and until the next call to NewFrame(). this is what you have to render.
    ...
    // Windows
    // - Begin() = push window to the stack and start appending to it. finish() = pop window from the stack.
    // - You may append multiple times to the same window during the same frame.
    // - Passing 'bool* p_open != NULL' shows a window-closing widget in the upper-right corner of the window,
    //   which clicking will set the boolean to false when clicked.
    // - Begin() return false to indicate the window is collapsed or fully clipped, so you may early out and omit submitting
    //   anything to the window. Always call a matching finish() for each Begin() call, regardless of its return value!
    //   [Important: due to legacy reason, this is inconsistent with most other functions such as BeginMenu/EndMenu,
    //    BeginPopup/EndPopup, etc. where the EndXXX call should only be called if the corresponding BeginXXX function
    //    returned true. Begin and BeginChild are the only odd ones out. Will be fixed in a future update.]
    // - Note that the bottom of window stack always contains a window called "Debug".
    IMGUI_API bool          Begin(const char* name, bool* p_open = NULL, ImGuiWindowFlags flags = 0);
    IMGUI_API void          End(bool fade_out);
    ...
    IMGUI_API void          EndChild(bool fade_out);

    // Windows Utilities
    // - 'current window' = the window we are appending into while inside a Begin()/finish() block. 'next window' = next window we will Begin() into.
    ...
    IMGUI_API void          SetWindowPos(const ImVec2& pos, ImGuiCond cond = 0);                        // (not recommended) set current window position - call within Begin()/finish(). prefer using SetNextWindowPos(), as this may incur tearing and side-effects.
    IMGUI_API void          SetWindowSize(const ImVec2& size, ImGuiCond cond = 0);                      // (not recommended) set current window size - call within Begin()/finish(). set to ImVec2(0,0) to force an auto-fit. prefer using SetNextWindowSize(), as this may incur tearing and minor side-effects.
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired cursor type, reset in ImGui::NewFrame(), this is updated during the frame. valid before on_directx(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
...
struct ImGuiIO
{
    ...
    bool        ConfigMacOSXBehaviors;          // = defined(__APPLE__) // OS X style: Text editing cursor movement using Alt instead of Ctrl, Shortcuts using Cmd/Super instead of Ctrl, Line/Text Start and finish using Cmd+Arrows instead of Home/finish, Double click selects by word instead of selecting whole text, Multi-selection in lists uses Cmd/Super instead of Ctrl (was called io.OptMacOSXBehaviors prior to 1.63)
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    // [OBSOLETE since 1.60+] Rendering function, will be automatically called in on_directx(). Please call your rendering function yourself now!
    // You can obtain the ImDrawData* by calling ImGui::GetDrawData() after on_directx(). See example applications if you are unsure of how to implement this.
    ...
    bool        KeyCtrl;                        // Keyboard alpha pressed: Control
    bool        KeyShift;                       // Keyboard alpha pressed: Shift
    bool        KeyAlt;                         // Keyboard alpha pressed: Alt
    bool        KeySuper;                       // Keyboard alpha pressed: Cmd/Super/Windows
    ...
    int         MetricsRenderVertices;          // Vertices output during last call to on_directx()
    int         MetricsRenderIndices;           // Indices output during last call to on_directx() = number of triangles * 3
...
struct ImGuiListClipper
{
    ...
    // If you don't specify an items_height, you NEED to call Step(). If you specify items_height you may call the old Begin()/finish() api directly, but prefer calling Step().
    ImGuiListClipper(int items_count = -1, float items_height = -1.0f)  { Begin(items_count, items_height); } // NB: Begin() initialize every fields (as we allow user to call Begin/finish multiple times on a same instance if they want).
    ~ImGuiListClipper()                                                 { IM_ASSERT(ItemsCount == -1); }      // Assert if user forgot to call finish() or Step() until false.
...
struct ImDrawPaint
{
    ...
    ImDrawPaint() : Col0(), Col1(), Start(), Extent() {};

    ImDrawPaint& SetLinearGradient(const ImVec2& start, const ImVec2& end, const ImVec4& col0, const ImVec4& col1, const float& angle = 0.f);
...
struct ImDrawList
{
    ...
    IMGUI_API void  PushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max, bool intersect_with_current_clip_rect = false);  // on_directx-level scissoring. This is passed down to your render function but not used for CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect() to affect logic (hit-testing and widget culling)
    ...
    IMGUI_API void  AddLineMultiColor(const ImVec2& p1, const ImVec2& p2, ImU32 col_begin, ImU32 col_end, float thickness = 1.0f);
    IMGUI_API void  AddRect(const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float rounding = 0.0f, ImDrawCornerFlags rounding_corners = ImDrawCornerFlags_All, float thickness = 1.0f);   // a: upper-left, b: lower-right (== upper-left + size), rounding_corners_flags: 4 bits corresponding to which corner to round
    IMGUI_API void  AddTriangleFilledMulticolor(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, ImU32 clr, ImU32 clr2, ImU32 clr3);
    IMGUI_API void AddInvertedRectFilled(const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float rounding, ImDrawCornerFlags rounding_corners);
    IMGUI_API void PathInvertedRect(const ImVec2& a, const ImVec2& b, ImU32 col, float rounding, ImDrawCornerFlags rounding_corners);
    IMGUI_API void  AddRectFilled(const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float rounding = 0.0f, ImDrawCornerFlags rounding_corners = ImDrawCornerFlags_All);                     // a: upper-left, b: lower-right (== upper-left + size)
    IMGUI_API void  AddRectFilledMultiColor(const ImVec2& p_min, const ImVec2& p_max, ImU32 col_upr_left, ImU32 col_upr_right, ImU32 col_bot_right, ImU32 col_bot_left);
    IMGUI_API void  AddRectFilledMultiColor(const ImVec2& p_min, const ImVec2& p_max, const ImDrawPaint& paint, float rounding = 0.f, ImDrawCornerFlags rounding_corners = 0xF);
    IMGUI_API void  AddQuad(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, ImU32 col, float thickness = 1.0f);
    IMGUI_API void  AddQuadFilled(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, ImU32 col);
    IMGUI_API void  AddQuadFilledMultiColor(const ImVec2& p_upr_left, const ImVec2& p_upr_right, const ImVec2& p_bot_right, const ImVec2& p_bot_left,
    ...
    IMGUI_API void  AddConvexPolyFilled(const ImVec2* points, int points_count, const ImDrawPaint& paint);
    ...
    inline    void  PathPaintConvex(const ImDrawPaint& paint) { AddConvexPolyFilled(_Path.Data, _Path.Size, paint); _Path.Size = 0; }
...
struct ImDrawData
{
    bool            Valid;                  // Only valid after on_directx() is called and before the next NewFrame() is called.
