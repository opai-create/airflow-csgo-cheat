...
//  2018-05-07: on_directx: Saving/restoring Transform because they don't seem to be included in the StateBlock. Setting shading mode to Gouraud.
...
#include "../../base/tools/default_vs.h"
...
// on_directx function.
// (this used to be set in io.RenderDrawListsFn and called by ImGui::on_directx(), but you can now call this directly from your main loop)
void ImGui_ImplDX9_RenderDrawData(const ImDrawData* draw_data)
{
    ...
    g_pd3dDevice->GetVertexDeclaration(&vert_dec);
    g_pd3dDevice->GetVertexShader(&vert_shader);
    ...
    g_pd3dDevice->SetVertexDeclaration(vert_dec);
    g_pd3dDevice->SetVertexShader(vert_shader);
