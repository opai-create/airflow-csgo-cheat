...
__forceinline c_material* create_material( const char* material_name, const char* material_data, const char* shader_type )
{
  ...
  func_ptrs::init_key_values( key_values, shader_type );
  func_ptrs::load_from_buffer( key_values, material_name, material_data, NULL, NULL, NULL );
  ...
  else
  {
    ...
    else if( chams.material == 6 && std::strlen( chams.chams_sprite ) > 0 )
    {
      ...
      if( var && old_base_texture != chams.chams_sprite )
      {
        ...
        temp_material = create_material( xor_c( "csgo_sprite_material" ), text_stream.str( ).c_str( ), xor_c( "VertexLitGeneric" ) );
...
bool c_chams::get_backtrack_matrix( matrix3x4_t* bones, c_csplayer* player, float& alpha )
{
  ...
  {
    auto animations = g_animation_fix->get_interp_record( player );
    if( animations.first == nullptr || animations.second == nullptr )
      return false;

    records_t *last_record = animations.first, *prelast_record = animations.second;
    if( last_record->dormant || prelast_record->dormant )
      return false;

    const auto next = last_record->abs_origin;

    float curtime = interfaces::global_vars->cur_time;
    auto delta = 1.f - ( curtime - last_record->interp_time ) / ( last_record->sim_time - prelast_record->sim_time );
    if( delta < 0.f || delta > 1.f )
      last_record->interp_time = curtime;

    delta = 1.f - ( curtime - last_record->interp_time ) / ( last_record->sim_time - prelast_record->sim_time );

    auto interpolated_position = math::interpolate( next, prelast_record->abs_origin, std::clamp( delta, 0.f, 1.f ) );

    std::memcpy( bones, prelast_record->sim_orig.bone, sizeof( matrix3x4_t [ 128 ] ) );
    math::change_matrix_position( bones, 128, prelast_record->abs_origin, interpolated_position );

    float distance = interpolated_position.dist_to( player->get_abs_origin( ) );
    if( distance <= 25.f )
...
void c_chams::add_shot_record( c_csplayer* player, matrix3x4_t* matrix )
{
  ...
  std::memcpy( data.bones, matrix, sizeof( matrix3x4_t ) * 128 );
...
void c_chams::on_post_screen_effects( )
{
  ...
  for( int i = 0; i < shot_records.size( ); ++i )
  {
    ...
    auto draw_model = [ & ]( c_float_color& clr, c_material* mat )
    {
      ...
      interfaces::model_render->forced_material_override( mat );
      original( interfaces::model_render, ctx, data.state, data.info, data.bones );
      interfaces::model_render->forced_material_override( nullptr );
...
bool c_chams::should_draw( )
{
  ...
  if( std::strstr( model->name, models_player.c_str( ) ) )
  {
    ...
    if( player == g_ctx.local )
    {
      ...
      auto& fake_matrix = g_local_animation_fix->local_info.matrix_fake;
      if( fake_matrix )
      {
        vector3d render_origin = g_ctx.local->get_render_origin( );

        math::change_matrix_position( fake_matrix, 128, vector3d( ), render_origin );
        this->draw_model( g_cfg.visuals.chams [ c_fake ], fake_matrix, alpha );
        math::change_matrix_position( fake_matrix, 128, render_origin, vector3d( ) );
      }
    ...
    else
    {
      ...
      if( g_ctx.lagcomp && g_cfg.visuals.chams [ c_history ].enable )
      {
...
void c_chams::on_draw_model_execute( draw_model_execute_fn original, void* ecx, void* edx, void* ctx, const draw_model_state_t& state, const model_render_info_t& info, matrix3x4_t* bone_to_world )
{
