...
std::string get_projectile_name( model_t* model, const int& class_id )
{
  switch( class_id )
  {
...
const char8_t* get_projectile_icon( model_t* model, const int& class_id )
{
  switch( class_id )
  {
  case( int )CBaseCSGrenadeProjectile:
...
bool c_grenade_warning::nade_path_t::should_draw( )
{
  ...
  last_path_pos = { };
  vector2d out{ };
  if( g_render->world_to_screen( path.back( ).first, out ) )
...
void c_grenade_warning::calc_nade_path( c_csplayer* entity )
{
  ...
  if( predicted_nades.find( handle ) == predicted_nades.end( ) )
  {
    predicted_nades.emplace( std::piecewise_construct, std::forward_as_tuple( handle ),
      std::forward_as_tuple( ( ( c_basecombatweapon* )entity )->thrower( ), get_weapon_index( client_class->class_id ), entity->origin( ), ( ( c_csplayer* )entity )->velocity( ), entity->grenade_spawn_time( ),
        math::time_to_ticks( ( ( c_csplayer* )entity )->simulation_time( ) - entity->grenade_spawn_time( ) ) ) );

    predicted_nades.at( handle ).preview_icon = ( const char* )get_projectile_icon( entity->get_model( ), client_class->class_id );
    predicted_nades.at( handle ).preview_name = get_projectile_name( entity->get_model( ), client_class->class_id );
  }
