    ...
    if( hit.alpha > 0.f )
    {
      ...
      if( g_render->world_to_screen( hit.pos, position ) )
      {
        if( g_cfg.misc.hitmarker & 1 )
        {
          ...
          g_render->line( position.x - 2, position.y - 3, position.x - 6, position.y - 6, clr.new_alpha( 255.f * hit.alpha ), 1.5f );
          g_render->line( position.x + 2, position.y + 3, position.x + 6, position.y + 6, clr.new_alpha( 255.f * hit.alpha ), 1.5f );
          g_render->line( position.x - 2, position.y + 3, position.x - 6, position.y + 6, clr.new_alpha( 255.f * hit.alpha ), 1.5f );
          g_render->line( position.x + 2, position.y - 3, position.x + 6, position.y - 6, clr.new_alpha( 255.f * hit.alpha ), 1.5f );
        }
        ...
        if( g_cfg.misc.damage )
        {
          ...
          g_render->string( position.x, position.y - 28.f, clr.new_alpha( 255.f * hit.alpha ), centered_x | outline_light, g_fonts.dmg, tfm::format( xor_c( "%d" ), hit.dmg ) );
        }
      }
    }
  }
