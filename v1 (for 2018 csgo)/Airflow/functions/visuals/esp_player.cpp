...
void render_main_esp( c_esp_store::player_info_t* info )
{
  ...
  auto esp_step = 1.f - ( info->health / 100.f );
  ...
  {
    if( cheat_tools::current_spread > 0.f )
    {
      g_render->circle( cheat_tools::spread_point.x, cheat_tools::spread_point.y, cheat_tools::current_spread, color( 255, 0, 255 ), 100 );
    }

    for( auto& p : cheat_tools::spread_points )
    {
...
if( enemy_esp.elements & 2 )
  ...
  if( info->dormant_alpha >= 0.9f && ( enemy_esp.elements & 512 ) )
  {
    ...
    for( int i = 0; i < 128; i++ )
      g_render->line( info->bone_pos_parent [ i ].x, info->bone_pos_parent [ i ].y, info->bone_pos_child [ i ].x, info->bone_pos_child [ i ].y, clr.new_alpha( clr.a( ) * info->dormant_alpha ), 1.f );
  }
...
void render_offscreen_esp( c_esp_store::player_info_t* info )
{
  ...
  vector2d position = { center.x + arrow_distance * std::cos( rotation ), center.y + arrow_distance * std::sin( rotation ) };

  vector2d points [ 3 ] = { vector2d( position.x - arrow_size, position.y - arrow_size ), vector2d( position.x + arrow_size, position.y ), vector2d( position.x - arrow_size, position.y + arrow_size ) };
  ...
  g_render->filled_triangle( points [ 0 ].x, points [ 0 ].y, points [ 1 ].x, points [ 1 ].y, points [ 2 ].x, points [ 2 ].y, clr.new_alpha( clr.a( ) * info->dormant_alpha ) );
