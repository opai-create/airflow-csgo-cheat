...
void c_utils::update_shot_time( )
{
  if( !g_ctx.weapon )
    return;

  if( g_anti_aim->is_fake_ducking( ) || g_ctx.weapon->is_pistols( ) || g_ctx.weapon->is_rifle( ) || g_ctx.weapon->is_misc_weapon( ) || g_exploits->hs_enabled( ) || !g_cfg.antihit.enable )
  {
    hold_fire_animation = false;
    return;
  }

  // with double tap your shot time is abit longer
  // because u shoot two shots (kinda stupid but YES)
  // add extra time for max shot time
  int max_shot_time = math::time_to_ticks( 0.2f ) + g_exploits->tickbase_offset( true );

  // time when was last shot
  int shot_diff = math::time_to_ticks( g_ctx.predicted_curtime - g_ctx.weapon->last_shot_time( ) );

  // we should stop hold firing animation on unlag
  // add choked comands to shot diff
  shot_diff += interfaces::client_state->choked_commands;
  shot_diff = std::max( 0, shot_diff );

  // now holding animation will be only when time is not up
  hold_fire_animation = shot_diff <= max_shot_time;
}

void c_utils::update_viewangles( )
{
  if( interfaces::client_state->choked_commands < 1 )
  {
  ...
  if( g_cfg.misc.clantag )
  {
    ...
    if( clantag_time != last_clantag_time )
    {
      if( next_update_time <= predicted_curtime || next_update_time - predicted_curtime > 1.f )
      {
        ...
        std::string tag_text = xor_str( "~ " ) + tag_text_2;

        marquee_text( tag_text_2 );
...
void c_utils::auto_revolver( )
{
  ...
  if( g_exploits->recharge || g_exploits->teleportshift || weapon->clip1( ) <= 0 )
  {
    last_checked = 0;
    tick_cocked = 0;
    tick_strip = 0;
    ...
    revolver_fire = false;
    g_exploits->teleportshift = false;
    return;
  }
...
{
  c_basecombatweapon* weapon = g_ctx.weapon;
  if( !g_ctx.local || !weapon || !g_ctx.local->is_alive( ) )
    return;

  short idx = weapon->item_definition_index( );
  if( idx == weapon_c4 || idx == weapon_healthshot || idx == weapon_revolver )
    return;

  if( weapon->is_auto_sniper( ) || weapon->is_misc_weapon( ) && !weapon->is_knife( ) )
    return;
...
bool c_utils::is_able_to_shoot( bool revolver )
{
  ...
  if( ( idx == weapon_glock || idx == weapon_famas ) && g_ctx.weapon->burst_shots_remaining( ) > 0 )
    return g_ctx.predicted_curtime >= g_ctx.weapon->next_burst_shot( );
  ...
  return g_ctx.predicted_curtime >= next_attack && g_ctx.predicted_curtime >= next_primary_attack;
}
...
void c_utils::on_predict_end( )
{
  ...
  auto weapon_info = g_ctx.weapon->get_weapon_info( );
  if( !weapon_info )
    return;

  *( bool* )( ( uintptr_t )weapon_info + 0x01BD ) = !g_cfg.misc.viewmodel_scope;
}
