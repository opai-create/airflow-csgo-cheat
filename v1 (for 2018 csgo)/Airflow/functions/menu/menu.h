...
private:
...
#if ALPHA
  std::string prefix = xor_str( "airflow (alpha)" );
#elif BETA
  std::string prefix = xor_str( "airflow (beta)" );
#else
  std::string prefix = xor_str( "airflow" );
#endif
  ...
  std::map< hash_t, item_animation_t > item_animations{ };
  std::array< LPDIRECT3DTEXTURE9, 7 > icon_textures{ };
  ...
  bool slider_int( const char* label, int* v, int v_min, int v_max, const char* format = xor_c( "%d" ) );
