...
constexpr int max_tabs = 6;

std::array< std::string, max_tabs > tabs = { xor_str( "Rage" ), xor_str( "Anti-hit" ), xor_str( "Visuals" ), xor_str( "Misc" ), xor_str( "Skins" ), xor_str( "Configs" ) };
...
void c_menu::init_textures( )
{
  ...
  if( !icon_textures [ 1 ] )
    add_texture_to_memory( g_render->direct_device, antihit_icon, sizeof( antihit_icon ), &icon_textures [ 1 ] );

  if( !icon_textures [ 2 ] )
    add_texture_to_memory( g_render->direct_device, visuals_icon, sizeof( visuals_icon ), &icon_textures [ 2 ] );

  if( !icon_textures [ 3 ] )
    add_texture_to_memory( g_render->direct_device, misc_icon, sizeof( misc_icon ), &icon_textures [ 3 ] );

  if( !icon_textures [ 4 ] )
    add_texture_to_memory( g_render->direct_device, skins_icon, sizeof( skins_icon ), &icon_textures [ 4 ] );

  if( !icon_textures [ 5 ] )
    add_texture_to_memory( g_render->direct_device, cfg_icon, sizeof( cfg_icon ), &icon_textures [ 5 ] );

  if( !avatar && g_ctx.avatar.size( ) )
    add_texture_to_memory( g_render->direct_device, g_ctx.avatar.data( ), g_ctx.avatar.size( ), &avatar );
}
...
void c_menu::draw_ui_background( )
{
  ...
  // header
  imgui_blur::create_blur( list, window_pos, ImVec2( window_pos.x + header_size.x, window_pos.y + header_size.y ), color( 255, 255, 255, window_alpha ).as_imcolor( ), 8.f, ImDrawCornerFlags_Top );
  ...
  // body
  imgui_blur::create_blur( list, window_pos + ImVec2( 0, 47 ), ImVec2( window_pos.x + 720, window_pos.y + 520 ), ImColor( 80, 80, 80, ( int )( window_alpha ) ), 8.f, ImDrawCornerFlags_Bot );
  ...
  // border
  list->AddRect( window_pos, ImVec2( window_pos.x + 720, window_pos.y + 520 ), color( 255, 255, 255, 12.75f * alpha ).as_imcolor( ), 8.f );
}
