...
Snowflake::Snowflake::Snowflake( float _minSize, float _maxSize, int _windowX, int _windowY, int _width, int _height )
{
  minSize = _minSize;
  maxSize = _maxSize;
  windowX = _windowX;
...
{
  auto snow_color = color( 255, 255, 255, 100 * g_menu->alpha );

  g_render->draw_list->AddCircleFilled( ImVec2( pos.x, pos.y ), radius, snow_color.as_imcolor( ) );
}
...
void Snowflake::CreateSnowFlakes( std::vector< Snowflake >& snow, uint64_t limit, float _minSize, float _maxSize, int _windowX, int _windowY, int _width, int _height, vec3 _gravity )
{
  gravity = _gravity;

  for( int i = 0; i < limit; i++ )
    snow.push_back( Snowflake( _minSize, _maxSize, _windowX, _windowY, _width, _height ) );
}
