...
void c_bone_builder::store( c_csplayer* player, matrix3x4_t* matrix, int mask )
{
  ...
  // fix model swaying
  bool add_sway = ( g_cfg.misc.animation_changes & 4 ) && g_utils->on_ground( );
  if( player == g_ctx.local && !add_sway )
  {
    auto lean_layer = &layers [ animation_layer_lean ];
    if( lean_layer )
      lean_layer->weight = 0.f;
  }
...
void c_bone_builder::setup( )
{
  ...
  c_ikcontext* ik_context = ( c_ikcontext* )animating->ik_context( );
...
{
  if( !*( bool* )( ( uintptr_t )player + 0x39E1 ) || !player->animstate( ) )
    return false;
