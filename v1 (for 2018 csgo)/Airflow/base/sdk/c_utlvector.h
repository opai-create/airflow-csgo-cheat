...
inline int calc_new_alloc_count( int nAllocationcount, int nGrowSize, int nNewSize, int nBytesItem )
{
  if( nGrowSize )
  {
...
public:
  ...
  void grow( int num )
  {
    ...
    int new_alloc_count = calc_new_alloc_count( m_alloc_count, m_grow_size, alloc_requested, sizeof( T ) );

    // if m_alloc_requested wraps index type interfaces, recalculate
    ...
    if( m_memory )
    {
      ...
      std::memcpy( ptr, m_memory, old_alloc_count * sizeof( T ) );
      m_memory = ( T* )ptr;
    }
    else
...
T& c_utlmemory< T, I >::operator[]( I i )
{
  return m_memory [ i ];
}

template < class T, class I >
const T& c_utlmemory< T, I >::operator[]( I i ) const
{
  return m_memory [ i ];
}
...
T& c_utlvector< T, A >::operator[]( int i )
{
  return m_memory [ i ];
}

template < typename T, class A >
const T& c_utlvector< T, A >::operator[]( int i ) const
{
  return m_memory [ i ];
}