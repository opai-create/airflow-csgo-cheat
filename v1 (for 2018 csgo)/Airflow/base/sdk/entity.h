...
public:
	...
	bool is_player() {
		...
		return g_memory->getvfunc<fn>(this, 152)(this);
	}

	bool is_weapon() {
		using fn = bool(__thiscall*)(void*);
		return g_memory->getvfunc<fn>(this, 160)(this);
	}
...
public:
	...
	netvar_ref(ik_context, int, 0x2570)
	...
	netvar_ref(collision_state, int, offsets::m_vphysicsCollisionState)
	netvar_ref(interpolation_tick, int, offsets::m_nFinalPredictedTick)
	netvar_ref(effects, int, 0xEC)
	netvar_ref(e_flags, int, 0xE4)
	...
	rnetvar(surface_friction, float, find_in_datamap(get_pred_desc_map(), xor_c("m_surfaceFriction")))
	rnetvar(next_attack, float, find_in_datamap(get_pred_desc_map(), xor_c("m_flNextAttack")))
	rnetvar(spawn_time, float, offsets::m_iAddonBits - 4)
	rnetvar(flash_time, float, offsets::m_flFlashtime)

	netvar_ref(last_bone_setup_time, float, 0x2914)
	netvar_ref(target_spotted, bool, offsets::m_bSpotted)
	...
	netvar_ref(grenade_spawn_time, float, 0x29B0)
	...
	netvar_ref(jiggle_bones_enabled, bool, 0x291C)
	...
	netvar_ptr_ref(animstate, c_animstate, 0x3874)
	netvar_ptr_ref(anim_overlay, c_animation_layers, 0x2970)

	netvar_ptr(bone_accessor, c_bone_accessor, offsets::m_nForceBone + 28)

	netvar_ref(bone_cache, c_utlvector<matrix3x4_t>, 0x2900)
	netvar_ref(model_recent_bone_counter, uint32_t, 0x2680)

	netvar_ref(cmd_context, cmd_context_t, 0x34D0)

	netvar_ptr(my_weapons, uint32_t, 0x2E08)

	__forceinline bool is_alive() { return this->life_state() == 0 && this->health() > 0; }
	...
	__forceinline void store_bone_cache(matrix3x4_t* matrix) {
		std::memcpy(matrix, this->bone_cache().base(), sizeof(matrix3x4_t) * this->bone_cache().count());
	}

	__forceinline void set_bone_cache(matrix3x4_t* matrix) {
		std::memcpy(this->bone_cache().base(), matrix, sizeof(matrix3x4_t) * this->bone_cache().count());
	}
	...
	void invalidate_bone_cache() {
		...
		this->bone_accessor()->readable_bones = this->bone_accessor()->writable_bones = -1;
	...
	bool is_bot() {
		return this->get_player_info().fakeplayer;
	}
	...
	float get_layer_sequence_cycle_rate(c_animation_layers* layer, int sequence) {
		...
		return g_memory->getvfunc<fn>(this, 217)(this, layer, sequence);
	}

	void update_clientside_animation() {
		using fn = void(__thiscall*)(void*);
		g_memory->getvfunc<fn>(this, 218)(this);
	}
	...
	void update_dispatch_layer(c_animation_layers* layers, c_studiohdr* hdr, int sequence) {
		...
		g_memory->getvfunc<fn>(this, 241)(this, layers, hdr, sequence);
	}
	...
	void setup_uninterpolated_bones( matrix3x4_t* matrix );
...
public:
	...
	float get_spread() {
		...
		return g_memory->getvfunc<fn>(this, 439)(this);
	}

	float get_inaccuracy() {
		using fn = float(__thiscall*)(void*);
		return g_memory->getvfunc<fn>(this, 469)(this);
	}

	void update_accuracy_penalty() {
		using fn = void(__thiscall*)(void*);
		return g_memory->getvfunc<fn>(this, 471)(this);
	}

	int get_zoom_fov(int lvl) {
		using fn = int(__thiscall*)(void*, int);
		return g_memory->getvfunc<fn>(this, 442)(this, lvl);
	}

	float get_zoom_time( int lvl )
	{
		using fn = float( __thiscall* )( void*, int );
		return g_memory->getvfunc< fn >( this, 443 )( this, lvl );
	}

	__forceinline bool is_heavy_pistols() {
		short idx = this->item_definition_index();
		return idx == weapon_deagle
			|| idx == weapon_revolver;
	}
	...
	std::string get_weapon_name() {
		...
		std::string name = weapon_info->sfui_name + 13;
